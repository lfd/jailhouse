...
If the system configuration has the flag JAILHOUSE_VIRTUAL_CONSOLE set, the
hypervisor console is available through /sys/devices/jailhouse/console.
Continuous reading of the hypervisor console is available through
/dev/jailhouse.
...
JAILHOUSE_VIRTUAL_CONSOLE set, the inmate is allowed to use the dbg_putc
hypercall to write to the hypervisor console. This is useful for debugging, as
the root cell is able to read the output of the inmate.
...
communication region.  If the non-root cell has the flag
JAILHOUSE_VIRTUAL_CONSOLE set, Jailhouse inmates will additionally write
to the Jailhouse virtual console.
