...
#define ATP(mode, pt)	((u64)mode << ATP_MODE_SHIFT | (u64)pt >> PAGE_SHIFT)

static inline void enable_mmu_satp(u8 mode, unsigned long pt)
{
	asm volatile("sfence.vma\n"
		     "csrw %0, %1\n"
		     "sfence.vma\n"
		     : : "i"(CSR_SATP), "rK"(ATP(mode, pt)) : "memory");
}

static inline void enable_mmu_hgatp(u8 mode, unsigned long pt)
{
	asm volatile("csrw %0, %1\n"
		     ".insn 0x22000073\n" /* hfence.vvma zero, zero */
		     : : "i"(CSR_HGATP), "rK"(ATP(mode, pt)) : "memory");
}
